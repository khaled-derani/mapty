...
class Workouts {
  ...
  constructor(coords, distance, duration) {
    ...
    this.duration = duration; // in distance
...
const run1 = new Running([39, -18], 5.2, 24, 178);
const cycling1 = new Cycling([39, -18], 27, 95, 523);
...
class App {
  ...
  _getPostion() {
    if (navigator.geolocation)
      navigator.geolocation.getCurrentPosition(
        ...
        function () {
          alert('there is a problem loading your info');
        }
      );
  }
  ...
  _loadMap(postion) {
    ...
    this.#map = L.map('map').setView(cords, 20);
  ...
  _newWorkout(e) {
    ...
    if (type === 'running') {
      ...
      if (
        ...
        return alert('the input have to be filled with postive numbers');
    ...
    //hide the form + clear input value
    this._hideForm();
...
// const map = L.map('map').setView([1, 1], 13);
